...
Interface parse(const std::string &path) {
  ...
  for (boost::filesystem::directory_iterator itr(cameras);
    ...
    Eigen::Matrix4d T;
    T << json["t_00"].get<double>(), json["t_01"].get<double>(),
        json["t_02"].get<double>(), json["t_03"].get<double>(),
        json["t_10"].get<double>(), json["t_11"].get<double>(),
        json["t_12"].get<double>(), json["t_13"].get<double>(),
        json["t_20"].get<double>(), json["t_21"].get<double>(),
        json["t_22"].get<double>(), json["t_23"].get<double>(), 0.0, 0.0, 0.0,
        1.0;
    Eigen::Matrix4d T_arkitcam_cam = Eigen::Matrix4d::Identity();
    T_arkitcam_cam.block<3, 3>(0, 0) =
    ...
    // pose.C = Interface::Pos3d(T(0, 3), T(1, 3), T(2, 3));
    auto C = Interface::Pos3d(T(0, 3), T(1, 3), T(2, 3));
    pose.SetTranslation(C);
